...
class ImportsController < ApplicationController
  ...
  def create_daily_figures(import_id)
    ...
    raw_r02 = RawRecruitment.select(:day, :pstatu_date).where(["import_id = ? AND pstatu_date != 0000-00-00 00:00:00", import_id]).group(:day).count(:pstatu_date)
    raw_recruited = RawRecruitment.select(:day, :datetime).where(["import_id = ? AND datetime != 0000-00-00 00:00:00", import_id]).group(:day).count(:datetime)
